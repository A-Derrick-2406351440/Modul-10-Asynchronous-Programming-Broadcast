...
- Saya menambahkan informasi from `derrick's computer` pada server dan client. Saya menambahkan `bcast_tx.send(format!("{addr} : {text}"))?;` agar memberikan pesan broadcast dengan format address dan textnya sehingga semua client mendapatkan informasi dari tiap address client lain. Lalu, saya juga menambahkan `println!("New connection from derrick's computer {addr:?}");` untuk menambahkan informasi connection ke server `derrick's computer` terjadi.